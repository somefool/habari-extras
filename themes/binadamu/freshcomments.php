				...
				<?php foreach ($freshcomments as $post): ?>
					<li>
						...
						<?php foreach ($post['comments'] as $comment): ?>
							<li><a style="color:<?php echo $comment['color']; ?>" href="<?php echo $comment['comment']->post->permalink; ?>#comment-<?php echo $comment['comment']->id; ?>" title="<?php printf(_t('Posted at %1$s', 'binadamu'), date('g:m a \o\n F jS, Y', strtotime($comment['comment']->date))); ?>"><?php echo $comment['comment']->name; ?></a></li>
						<?php endforeach; ?>
						</ul>
					</li>
				<?php endforeach; ?>
