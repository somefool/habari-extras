<?php
	if (count($years) > 0) {
		...
		foreach ($years as $year => $months) {
			printf('<li class="year"><a href="%1$s" rel="bookmark">%2$s</a><ul class="breezy-yearly-archive">',
					URL::get('display_entries_by_date', array('year' => $year)),
					$year);
			if ($show_monthly_post_count) {
				foreach ($months as $month => $count) {
					if ($count > 0) {
						printf('<li class="month"><a href="%1$s" rel="bookmark">%2$s</a> <span class="post-count" title="%3$s">%4$d</span></li>',
				...
				foreach ($months as $month => $count) {
					if ($count > 0) {
						printf('<li class="month"><a href="%1$s" rel="bookmark">%2$s</a></li>',
								URL::get('display_entries_by_date', array('year' => $year, 'month' => $month)),
								Utils::locale_date($month_format, mktime(0,0,0,$month,1)));
					} else {
