		...
		<div id="entry-<?php echo $post->slug; ?>" class="hentry entry <?php echo $post->statusname , ' ' , $post->tags_class; ?>">
			<div class="entry-head">
				...
				<ul class="entry-meta">
					...
					<li class="comments-link"><a href="<?php echo $post->permalink; ?>#comments" title="<?php _e('Comments to this post') ?>"><?php printf( _n('%1$d Comment', '%1$d Comments', $post->comments->approved->count), $post->comments->approved->count ); ?></a></li>
<?php if ( is_array( $post->tags ) ) { ?>
					<li class="entry-tags"><?php echo $post->tags_out; ?></li>
<?php } ?>
<?php if ( isset($user) && $user ) { ?>
					<li class="entry-edit"><a href="<?php URL::out( 'admin', 'page=publish&slug=' . $post->slug); ?>" title="<?php _e('Edit post') ?>"><?php _e('Edit') ?></a></li>
<?php } ?>
				</ul>
			</div>
