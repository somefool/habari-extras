...
{
	// array $files Logfiles to use
	private $files = array();

	// integer $minScreen Screen size filter
	public $minScreen;

	// integer $maxScreen Screen size filter
	public $maxScreen;
	...
	public function drawPixels($image)
	{
		 ...
		 * just use the value stored in the temp files
		 */
		if ($image !== 0)
		{
			if (file_exists($this->cache.sprintf($this->file, $image).'_log'))
			{
				...
				while (feof($f) === false)
				{
					...
					if (count($click) === 3)
					{
						...
						{
							/**
							 * Apply a calculus for the step,
							 * with increases the speed of rendering:
							 * step = 3, then pixel is drawn at x = 2 (center of a 3x3 square)
							 */
							$x -= $x % $this->step - $this->startStep;
							$y -= $y % $this->step - $this->startStep;

							/**
							 * Add 1 to the current color of this pixel
							 * (color which represents the sum of clicks on this pixel)
							 */
		...
		$leftWidth = 10;//(int) $this->layout[1];
		$centerWidth = 0;//(int) $this->layout[2];
		$rightWidth = 0;//(int) $this->layout[3];
		if ($leftWidth !== 0 && $centerWidth !== 0 && $rightWidth === 0)
		{
			/**
			 * Fixed left menu and fixed center ?
			 * Fall back to a fixed left menu only
			 */
		...
		{
			/**
			 * Fixed right menu and fixed center ?
			 * Fall back to a fixed right menu only
			 */
		...
		for ($file = 0; $file < count($this->files); $file++)
		{
			...
			while (true)
			{
				...
				preg_match_all('~^(\d+)\|(\d+)\|(\d+)\|(\d+)$~m', $buffer, $clicks);
				$buffer = '';
				...
				for ($i = 0, $max = count($clicks[1]); $i < $max; $i++)
				{
					...
					/**
					 * X is not in the range of sizes,
					 * or right click for heatmap,
					 * or X is greater than screen size,
					 * the website is too large for the window,
					 * so we don't know where the click is... ignore those clicks
					 */
					if ($x <= $this->minScreen || $x > $this->maxScreen || $this->heatmap && $c === false || $x > $w)
						continue;
					...
					if ($w <= $totalWidth) {
						/**
						 * Display's width is less than fixed content's one,
						 * X is absolute
						 */
					}
					...
					elseif ($leftWidth === 0 && $centerWidth === 0 && $rightWidth !== 0) {
						...
						if ($w - $x <= $rightWidth)
							 ...
							 * X is good, but relative to the right border
							 */
							$x = $this->width - ($w - $x);
