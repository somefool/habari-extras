		...
		if ( $plugin_id == $this->plugin_id ) {
			switch ( $action ) {
				case 'Options':
					...
					$grav_settings_default= $ui->add( 'text', 'default', '<dl><dt>Default Gravatar</dt><dd>An optional "default" parameter may follow that specifies the full, URL encoded URl, protocol included of a GIF, JPEG or PNG image that should be returned if either the request email address has no associated gravatar, or that gravatar has a rating higher than is allowed by the "rating" parameter.</dd></dl>', Options::get( 'gravatar:default' ) );
					$grav_settings_size= $ui->add( 'text', 'size', '<dl><dt>Size</dt><dd>An optional "size" parameter may follow that specifies the desired width and height of the gravatar. Valid vaues are from 1 to 80 inclusive. Any size other than 80 will cause the original gravatar image to be downsampled using bicubic resampling before output.</dd></dl>', Options::get( 'gravatar:size' ) );
					$grav_settings_rating= $ui->add( 'select', 'rating', '<dl><dt>Rating</dt><dd>An optional "rating" parameter may follow with a value of [ G | PG | R | X ] that determines the highest rating (inclusive) that will be returned.</dd></dl>', '', '' );
					$grav_settings_rating->options= array( 'G' => 'G', 'PG' => 'PG', 'R' => 'R', 'X' => 'X' );
					$grav_settings_rating->selected= Options::get( 'gravatar:rating' );
